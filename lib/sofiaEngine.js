...
export async function runSofiaTurn(
  ...
  const model = settings.openai?.model || process.env.OPENAI_MODEL || 'gpt-4.1-mini';

  if (!apiKey) {
    console.error('Chave da OpenAI não configurada para esta empresa — defina no /superadmin.');
