...
// Mesma chave de chaveIdentidade(), mas a partir de um item JÁ unificado
// (buildProfissionaisUnificados) — usada fora deste módulo (ver
// app/api/admin/profissionais/route.js, MedicosDirectory) pra identificar
// de forma estável qual profissional o admin ativou/desativou o link, sem
// depender de um id próprio (não existe um no ClinVida).
export function identidadeProfissional(prof) {
  return chaveIdentidade(prof) || `solo:${prof.slug}`;
}
